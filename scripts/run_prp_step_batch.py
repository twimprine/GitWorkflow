...
def main() -> int:
    ...
    for item in items:
        ...
        data = payload
        if isinstance(data, dict) and ("content" in data or "outputs" in data):
            dest = None
        ...
        # Fallback: if we have a dict but not recognized wrapper keys, still save it as the draft in JSON mode
        if isinstance(data, dict) and args.draft_ext == "json":
            dest = _normalize_draft_path(f"{slug}", slug, step_id, batch_ts, args.draft_ext)
            _ensure_parent_dir(dest)
            Path(dest).write_text(json.dumps(data, indent=2), encoding="utf-8")
            print(f"Saved draft (fallback) -> {dest}")
            continue
